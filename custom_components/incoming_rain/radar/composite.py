...
def _tile_upper_left(tx: int, ty: int, zoom: int) -> tuple[float, float]:
    """Return (lat, lon) of tile upper-left corner."""
    n = 2**zoom
...
def filter_precipitation_pixels(rgba_array: np.ndarray) -> np.ndarray:
    ...
    non_precip = ~alpha_mask | (best_dist > _MAX_COLOUR_DISTANCE)
...
async def render_composite(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for ty in range(tile_y_min, tile_y_max + 1):
            for tx in range(tile_x_min, tile_x_max + 1):
                url = f"{_MAP_TILE_BASE}/{map_zoom}/{tx}/{ty}.png"
                try:
                    ...
                    pass  # missing map tile - leave black
        ...
        # Fetch radar tiles at zoom 7 (max RainViewer supports)
        radar_zoom = _RAINVIEWER_ZOOM
        radar_centre_px, radar_centre_py = _lat_lon_to_pixel(lat, lon, radar_zoom)

        # We need to figure out which radar tiles cover our map viewport
        # Convert map viewport corners to lat/lon, then to radar tile coords
        map_n = 2**map_zoom
        ...
        for ty in range(radar_ty_min, radar_ty_max + 1):
            for tx in range(radar_tx_min, radar_tx_max + 1):
                url = (
                    ...
                    f"/{TILE_SIZE}/{radar_zoom}/{tx}/{ty}/{_RAINVIEWER_COLOUR_SCHEME}/0.png"
                )
                try:
                    tile = await _fetch_tile(session, url)
    ...
    # Filter to precipitation-only pixels
